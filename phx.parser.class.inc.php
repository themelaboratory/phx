...
#   Modified by Milos Djordjevic (milos@themelaboratory.com)
        ...
        if (preg_match_all('~:([^:=]+)(?:=`(.*?)`(?=:[^:=]+|$))?~s', $modifiers, $matches)) {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                switch ($modifier_cmd[$i]) {
                    ...
                    case "in":
                    case "isin":
                        $condition[] = intval(in_array($output, explode(',', $modifier_value[$i])));
                        break;
                    case "contains":
                        $condition[] = intval(false !== strpos($output, $modifier_value[$i]));
                        break;
                    case "notequals":
                    case "isnot":
                    case "isnt":
                    ...
                    case "htmlquot":
                    case "htmlspec":
                        $output = htmlspecialchars($output, ENT_COMPAT, $modx->config['modx_charset'], false);
                        break;
                    case "jsonenc":
                    case "jsonencode":
                    ...
                    case "rtrim":
                    case "trim":
                        $output = $modifier_cmd[$i]($output, $modifier_value[$i]);
                        break;
                    #####  Special functions
                    case "math":
                    ...
                    case "isntempty":
                    case "notempty":
                        if (trim($output) != '') $output = $modifier_value[$i];
                        break;
                    case "date":
